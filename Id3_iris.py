#Akshansh Chaudhry                       #
#ID3 DecisionTree algorithm on iris data #
##########################################
...
def accuracy_for_predicted_values(test_class_names1,l):
    ...
    for i in range(len(test_class_names1)):
        if(test_class_names1[i] == l[i]):
            ...
            false_count += 1
    return true_count, false_count, float(true_count) / float(len(l))
...
def printTree(tree):
    printNode(tree)
    # print("Accuracy is: %f", self.accuracy(self.train_data))
    # self.Accuracy()
    ...
    if not node.is_leaf():
        if node.theta is None:
            ...
            if leftChild.is_leaf():
                ...
                print ("%s %s <= %s : %s"%(indent, node.root_value, str(node.theta), leftChild.root_value))

            else:
                # print(indent + node.root_value + " <= " + str(node.threshold)+" : ")
                print ("%s %s <= %s : "%(indent, node.root_value, str(node.theta)))
